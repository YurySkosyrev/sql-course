...
Non Repeatable Read - происходит, когда первая транзакция читает одни и те же данные дважды, но после первого прочтения вторая транзакция изменияет (update) эти же данные и делает коммит, вследствии чего вторая выборка в первой транзакции вернет другой результат.

![alt text](img/non_repeatable_read.jpg "dbms")

Cуществует особый случай Non Repeatable Read, называемый last commit wins, когда обе транзакции читают одни и те же данные, но первая успевает изменить и закомитить их раньше, чем произойдет изменение и коммит во второй транзакции, вследствие чего изменение первой транзакции теряются.

![alt text](img/who_first_win.jpg "dbms")

Fantom Read - происходит, когда первая транзакция читает одни и те же данные дважды, но после первого прочтения вторая транзакция добавляет новые строки или удаляет старые и делает коммит, вследствие чего вторая выборка в первой транзакции вернет другой результат (разное колличество записей)

Похоже на non repeatable read, но здесь не insert и delete.

![alt text](img/fantom_read.jpg "dbms")

В случае non-repeatable read можно заблокировать обновляемые строки, а в случае fantom read приходится блокировать всю таблицу.

![alt text](img/fantom_read.jpg "dbms")

![alt text](img/isolation_levels.jpg "dbms")

Чем выше уровень, тем больше уменьшается возможность работать с таблицей параллельно. В случае Serializable - только последоваетельная работа.

![alt text](img/postgres_isolate_level.jpg "dbms")

Read commited по умолчанию в Postgres.

## Транзакции в Postgres

Lost Update и Dirty Read никак нельзя воспроизвести, так как минимальный уровень Read Commited.

show transaction isolation level - показать уровень изолированности транзакции.

show default_transaction_isolation - показать уровень изолированности по умолчанию.

begin - открыть транзакцию. Если не прописать, то Postgres по умолчанию откроет транзакцию.

Для решения проблемы Non-repeatable read нужно повысить уровень изолированности транзакции

begin transaction isolation level repeatable read

Если в одной транзакции сделан Update, то при попытке Update в другой транзакции сессия зависнет до тех пор, пока в первой не будет Commit в первой транзакции. При этом во второй транзакции возникнет ошибка и её придется откатить Rollback, так как Commit из-за ошибки сделать не удасться.

Для поддержания уровня изоляции repatable read делает snap-shot копия данных, которые считывались в текущей транзакции. Поэтому чем больше данных запрашивается, тем больше ресурсов требуется на создание копии. Следовательно не стоит делать транзакций с большим числом изменений данных, т.к. это ресурсоъекая операция.

Для решения проблемы Fantom Read необходимо установить уровень транзакции Serializable

begin transaction isolation level serializable

При этом так же используются snap-shot копии и дополнительно предикатные блокировки.

Кроме повышения уровня транзакции можно делать так называемые row-level lock

select * from aircraft for update - данные строки будут заблокированы для update в других транзакциях, они будут ожидать. Реже используются row-level lock.

## Тригер

Триггер - сущность БД, предназначенная для каких-то действий до, после, вместо процедур insert, update, delete.

Триггеры используются для аудита.

Есть операция Truncate, она очищает всю таблицу и работает быстрее Delete, т.к. Delete логгирует все строки в которые были внесены изменения.

Функция, которая вызывается у триггера пишется на специальном процедурном языке для каждой конкретной БД. Для Postgres - plpgsql.
...
Data WareHouse - хранилище только нужных и важных данных для компании, на основании которых будет производится анализ и последующие принятие бизнес решений.

![alt text](img/dwh.jpg "dbms")

Основные отличия от БД

1. Типы хранимых данных. 

Обычные СУБД хранят данные строго для определенных подсистем (книги, перелеты, товаря). DWH, как правило, хранят информацию разных областей - там найдутся данные и по книгам, и по перелетам и по товарам.

2. Обмен данными

Обычная СУБД используется для хранения только актуальной информации, нужной в данный момент для функционирования определенной системы. В DWH пишутся не столько копии актуальных данных (хотя можно в копию целой базы хранить, если потребуется для аналитики), сколько исторические данные и агрегированные значения.

3. Процесс заполнения данными

Информация обычно сразу попадает в привычные нам СУБД, а уже оттуда переползает в DWH, предварительно пройд обработку и преобразование (ETL - Extract -> Transform -> Load)
Хранилище данных (DWH), по сути, отражает состояние других БД уже после того, как вносятся изменения в них.

BigQuery - это одна из лучших БД для аналитики от компании Google (есть аналоги от Microsoft, Amazon и так далее)

При вводе данных создаётся несколько таблиц вместо одной, которые благодаря технологии Cloud позволяют распараллелить работу с данными, значительно ускорив её.

## NoSQL. Теорема CAP

Обычно БД устанавливается на один сервер, где с ней происходит работа. Чтобы при отказе сервера работа могла быть продолжена БД копируется на несколько разных серверов. Чем больше копий - тем больше необходимо вычислительных мощностей.

**Репликация данных** - это процесс, под которых понимается копирование данных из одного источника на другой (или на множество других) ои наоборот.

Виды репликации:
- Синхронная (в той же транзакции). При этом запрос отправляется на все копиии БД и пользователь вынужден ждать. Используется двухфазный коммит.

- Асинхронная (спустя некоторое время и в разных транзакциях) При этом пользователь может попасть на копию в которую ещё не внесены изменения с основной БД.

![alt text](img/replication.jpg "dbms")

Master - Slave. Master принимает запросы на чтение и изменения данных. Slave только на чтение. При изменении данных они передаются slave.

Master - Master - каждая реплика самостоятельная.

**Фрагментация или сегментация (Sharding) БД** - это процесс разбиения одной большой БД на несколько более мелких автономных разделов. Может применяться хэш-таблица.

Каждый раздел имеет одинаковую схему и столбцы, но разные строки.

Соответственно, данные, хранящиеся в каждом из них уникальны и не зависят от данных, хранящихся в других разделах.

Совмещая репликацию и фрагментацию мы получаем распределенные БД (distributed database) - БД, составные части которой размещаются в различных узлах компьютерной сети в соответствии с каким-либо критерием.

**Теорема CAP** - это теорема, утверждающая, что при разработке распределенной системы (БД) вы не можете достичь одновременно трёх свойств. Одновременно достижимы только два.

- **согласованность (consistency)** - каждое чтение даёт нам самую последнюю запись
- **доступность (availability)** - система всегда успешно выполняет запросы (на чтение и запись), даже если имеются упавшие (не работающие) узлы
- **терпимость к разделению сети (partition tolerance)** - даже если между узлами пропала связь, они продолжают работать независимо друг от друга и выполнять запросы.

**NoSQL (нереляционные БД)** - базы данных, специально предназначенные для работы в распределенных системах, т.е. с большим объемом данных, не помещающимися на один узел.
...
- Графовые БД (Neo4j)
- БД в памяти "in-memory" (Apache Ignite)
- Поисковые БД (Elasticsearch)

Evantually Consistency - согласованность в конечном счете.
